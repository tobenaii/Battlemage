...
namespace Battlemage.Networking.Utilities
{
    public static class NetworkInputUtilities
    {
        private const float DefaultWrapAroundValue = 1000f;

        public static void GetCurrentAndPreviousTick(NetworkTime networkTimeSingleton, out NetworkTick currentTick,
        ...
        public static void GetCurrentAndPreviousTickInputs<T>(DynamicBuffer<InputBufferData<T>> inputsBuffer,
            NetworkTick currentTick, NetworkTick previousTick, out T currentTickInputs, out T previousTickInputs)
        ...
        public static void AddInputDelta(ref float input, float addedDelta,
            float wrapAroundValue = DefaultWrapAroundValue)
        {
            input = math.fmod(input + addedDelta, wrapAroundValue);
        }

        public static float GetInputDelta(float currentTickValue, float previousTickValue,
            float wrapAroundValue = DefaultWrapAroundValue)
        {
